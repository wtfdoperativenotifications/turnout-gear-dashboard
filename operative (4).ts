import type { DashboardPayload, DueStatus, GearAsset, LocationType, MemberCoverage, ReplacementItem, SourceDiagnostic } from "./types";
...
function normalize(record: JsonRecord, index: number): GearAsset {
  ...
  return {
    ...
    size: firstText(record, ["size", "assetSize", "coatSize", "pantSize", "itemSize"]),
...
function dashboard(gear: GearAsset[], source: string, diagnostic: SourceDiagnostic, lookaheadDays: number): DashboardPayload {
  ...
  return {
    ...
    replacementPriority: replacementPriority(gear), decommissionForecast: forecast
  };
}
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/api/health") return json({ ok: true, service: "turnout-gear-dashboard", version: "11.0.0", time: new Date().toISOString() });
    if (url.pathname === "/api/data-explorer") { const result = await loadDataExplorer(env); return json(result.body, { status: result.status }); }
    if (["/api/dashboard","/api/maintenance-due","/api/assets","/api/members","/api/warehouse","/api/replacement-plan","/api/source-diagnostic"].includes(url.pathname)) {
      const lookaheadDays = Math.max(1, Math.min(365, Number(url.searchParams.get("days") || env.MAINTENANCE_LOOKAHEAD_DAYS || 30)));
      const result = await loadGear(env);
      if (url.pathname === "/api/source-diagnostic") return json(result.diagnostic, { status: result.diagnostic.ok ? 200 : 502 });
      const payload = dashboard(result.gear, result.source, result.diagnostic, lookaheadDays);
